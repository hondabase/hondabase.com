    ...
    @else
        ...
        <section class="dash-section">
            <div class="garage-head">
                <h2 class="section-head">{{ __('Garage') }}</h2>
        ...
        {{-- ---------- Following feed ---------- --}}
        <section class="dash-section">
            <h2 class="section-head">{{ __('From things you follow') }}</h2>
                    ...
                    @foreach ($follows as $f)
                        <span class="chip chip-followed" wire:key="fol-{{ $f->id }}">
                            ...
                            <button type="button" wire:click="unfollow('{{ $f->kind }}:{{ $f->value }}')"
                                title="{{ __('Unfollow') }}" aria-label="{{ __('Unfollow :label', ['label' => $f->label ?: $f->value]) }}">&times;</button>
                        </span>
                    @endforeach
                </div>
            @endif
                ...
                <div class="grid">
                    @foreach ($feed as $a)
                        <a class="card" href="{{ '/' . $a->type . '/' . $a->category . '/' . $a->slug }}" wire:navigate wire:key="feed-{{ $a->id }}">
                            ...
                            @if ($a->updated_at)<span class="card-meta">{{ __('Updated :date', ['date' => $a->updated_at->isoFormat('LL')]) }}</span>@endif
                        </a>
                    @endforeach
                </div>
            @endif
                ...
                <div class="saved-list">
                    @foreach ($favorites as $a)
                        <div class="saved-row" wire:key="fav-{{ $a->id }}">
                            ...
                            <button type="button" class="btn-link danger" wire:click="unsave({{ $a->id }})">{{ __('Remove') }}</button>
                        </div>
                    @endforeach
                </div>
            @endif
