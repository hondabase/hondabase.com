...
class LintArticles extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($types as $type) {
            ...
            foreach ($this->articleDirs($typePath) as $article) {
                $category = $article['category'];
                $slug = $article['slug'];
                $articleDir = $article['dir'];
                $checkedCount++;
                ...
                if (! empty($fm)) {
                    ...
                    if (isset($fm['applies_to'])) {
                        if (! is_array($fm['applies_to'])) {
                            ...
                            $allowedAppliesToKeys = ['make', 'models', 'model', 'chassis', 'trims', 'trim', 'engines', 'years', 'scope'];
                ...
                $isElectronics = $category === 'electronics' || str_starts_with($category, 'electronics/');
                if ($type === 'cars' && $isElectronics && $isPgmfiPort && empty($fm['sources'])) {
    ...
    /**
     * Find article bundle directories below a type root. Directories with Markdown are article
     * leaves; directories without Markdown are branch folders that may contain deeper articles.
     *
     * @return list<array{category: string, slug: string, dir: string}>
     */
    private function articleDirs(string $typePath): array
    {
        $articles = [];

        $walk = function (string $dir, string $category) use (&$walk, &$articles): void {
            foreach ((array) glob("{$dir}/*", GLOB_ONLYDIR) as $child) {
                $slug = basename($child);
                if ((array) glob("{$child}/*.md") !== []) {
                    $articles[] = [
                        'category' => $category,
                        'slug' => $slug,
                        'dir' => $child,
                ...
                $walk($child, $category === '' ? $slug : "{$category}/{$slug}");
            }
        };
