...
- **Discord Integration & GitHub Webhooks:** Created root-level public Discord text channel `#github` (ID `1534826763425681499`, ordered at position 3) and provisioned GitHub repository webhooks across all 11 repositories in the `hondabase` organization (`hondabase.com`, `articles`, `discord-bot`, `manuals.hondabase.com`, `files-archive`, `.github`, `d-series`, `manuals`, `ecu-pinouts`, `vin-decoder`, `hondarulez`) via `gh api`.
...
- **2026-06-20** - Restored the shared static stylesheet at `public/assets/base.css` for sibling/static sites such as `manuals.hondabase.com`, but made it a generated Tailwind/Vite artifact rather than a hand-written public file. Added `resources/css/base.css` as a Vite CSS entry and `scripts/publish-base-css.mjs`; `pnpm build` now compiles the Tailwind bundle and publishes the stable compatibility path. Updated `manuals.hondabase.com` on `nasa` to load `https://www.hondabase.com/assets/base.css` again.
- **2026-06-20** - Added manuals archive favorites. `www` now exposes a credentialed, CORS-limited `manuals/favorites` endpoint backed by `manual_favorites`, and the dashboard shows/removes saved manuals alongside saved articles. The static manuals site renders per-file save buttons, uses the shared `.hondabase.com` login/session, and sends users through `/auth/login?return=<manuals-url>` so Discord login returns them to the manuals folder they came from.
- **2026-06-21** - Let rendered article bodies use the full site container width by removing the extra `72ch` cap from `.prose-article`; wide tables, code blocks, images, and embedded article widgets now expand to the article column instead of stopping short inside the main wrapper.
- **2026-06-21** - Fixed article KaTeX initialization after Livewire navigation. The article bundle now renders math immediately when loaded and again on `livewire:navigated`, guarded per `.prose-article` so formulas work on first article view without needing a browser refresh.
