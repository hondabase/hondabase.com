<div class="notif" x-data="{ open: false }" @click.outside="open = false" @keydown.escape="open = false">
    <button type="button" class="notif-btn" @click="open = !open" :aria-expanded="open.toString()" aria-label="Notifications">
    ...
    <div class="notif-panel" x-show="open" x-transition x-cloak>
        <div class="notif-head">
            ...
            @if ($this->unread > 0)
                <button type="button" class="btn-link" wire:click="markAllRead">Mark all read</button>
            @endif
        </div>
        ...
        @forelse ($this->items as $n)
            <button type="button" class="notif-item @if(is_null($n->read_at)) is-unread @endif"
                ...
                <span class="notif-title">{{ data_get($n->data, 'is_new') ? 'New' : 'Updated' }}: {{ data_get($n->data, 'title') }}</span>
        ...
        @empty
            <p class="notif-empty">Nothing yet. Follow a category, engine or chassis and new
            articles will show up here.</p>
        @endforelse
    </div>
</div>
